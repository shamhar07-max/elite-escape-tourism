import { mkdirSync, writeFileSync, readdirSync, statSync, existsSync, readFileSync } from "fs";
...
// Once scripts/localize-images.mjs has downloaded an id+width pair, use that local file
// instead of hotlinking Unsplash (which throttles/blocks high-volume unauthenticated use).
const IMG_MANIFEST_PATH = join(root, "assets", "img", "manifest.json");
const IMG_MANIFEST = existsSync(IMG_MANIFEST_PATH) ? JSON.parse(readFileSync(IMG_MANIFEST_PATH, "utf8")) : {};
const U = (id, w) => IMG_MANIFEST[`${id}-${w}`] || `https://images.unsplash.com/photo-${id}?q=80&w=${w}&auto=format&fit=crop`;
...
<link rel="preload" href="/assets/fonts/figtree-variable-latin.woff2" as="font" type="font/woff2" crossorigin>
...
const header = (cur) => `<body class="grain">${NOSCRIPT}<div class="scroll-bar" id="scrollBar"></div><a class="skip" href="#main">Skip to content</a>
...
const heroBand = (kicker, h1, lede, imgUrl, alt, cta = "") => { const hid = (imgUrl.match(/photo-([\w-]+)/) || imgUrl.match(/\/([\w.-]+?)-\d+\.\w+(?:$|\?)/) || [])[1] || ""; return `<section class="hero" style="min-height:64svh"><div class="hero-slides"><div class="hero-slide on">${HEROIMG(hid, alt, 1800, 1000, true)}</div></div><div class="hero-glow"></div><div class="wrap hero-inner" style="padding:150px 0 52px"><p class="hero-kicker"><i></i>${kicker}</p><h1>${h1}</h1><p class="hero-sub">${lede}</p><div class="hero-actions">${cta}</div></div></section>`;};
...
put("visa.html", page("Visa Assistance in Dubai — 49 Countries | Elite Escape", "USA, UK, Schengen, Canada, Australia & more. Checklists, appointments, file review on WhatsApp. UAE tourist visas in 24–72h.", "/visa.html",
  heroBand("Visa atelier", "The stamp, <span class='hl'>without</span> the stress.", "Pick your country. A specialist replies with a dated checklist.", U("1488646953014-85cb44e25828", 1800), "Traveller planning the journey", `<a class="btn btn-white" href="${WA}?text=Visa%20help" target="_blank" rel="noopener">Ask a specialist <span class="arr">→</span></a>`) +
...
for (const g of guides) {
  ...
  put(`guides/${g}.html`, page(`${pretty(g)} (2026) — Dubai Guide | Elite Escape`, `${pretty(g)}: honest costs, months, visa notes and mistakes to avoid — by Dubai specialists. WhatsApp +971 55 575 3133.`, `/guides/${g}.html`,
    `<section class="hero" style="min-height:52svh"><div class="hero-slides"><div class="hero-slide on">${HEROIMG(gi, ga, 1800, 900, true)}</div></div><div class="wrap hero-inner" style="padding:150px 0 50px"><p class="hero-kicker"><i></i>Field guide · 2026</p><h1 style="text-transform:capitalize">${g.replace(/-/g, " ")}.</h1></div></section>` +
...
const renderBlogPost = (p, i, all) => {
  ...
  return page(`${p.title} | Elite Escape`, p.desc, `/blog/${p.slug}.html`,
    `<section class="hero" style="min-height:56svh"><div class="hero-slides"><div class="hero-slide on">${HEROIMG(p.img, p.alt, 1800, 950, true)}</div></div><div class="wrap hero-inner" style="padding:150px 0 50px"><p class="hero-kicker"><i></i>${p.cat} · ${pretty} · ${read} min read</p><h1 style="font-size:clamp(2rem,4.6vw,3.6rem)">${p.title}</h1></div></section>` +

...
(function(){
  ...
  $$(".rv,.rv-l").forEach(el=>io.observe(el));
  ...
  if(fab&&panel){
    ...
    const say=(html,who="bot")=>{const d=document.createElement("div");d.className="msg "+who;d.innerHTML=html;body.appendChild(d);body.scrollTop=body.scrollHeight;return d};
    ...
    const ask=text=>{const q=(text||"").trim();if(!q)return;say(q.replace(/</g,"&lt;"),"user");input.value="";
      const t=document.createElement("div");t.className="msg bot typing";t.innerHTML="<i></i><i></i><i></i>";body.appendChild(t);body.scrollTop=body.scrollHeight;
